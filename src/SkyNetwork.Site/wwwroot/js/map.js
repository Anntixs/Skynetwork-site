...
(function () {
  ...
  const plane = heading => L.divIcon({
    ...
    html: `<svg width="20" height="20" viewBox="0 0 24 24" style="transform:rotate(${heading ?? 0}deg)"><path fill="#0e1726" d="M12 2c.8 0 1.3.7 1.3 1.6v5.6l7.7 4.6v2l-7.7-2.3v4.4l2.2 1.7v1.6L12 20.2l-3.5 1v-1.6l2.2-1.7v-4.4L3 15.8v-2l7.7-4.6V3.6C10.7 2.7 11.2 2 12 2z"/></svg>`
  });
  ...
  async function refresh() {
    ...
    for (const c of data.controllers) {
      ...
      L.circle(ll, { radius: radiusNm(c.facility) * 1852, color: '#1f5fbf', weight: 1, fillOpacity: .06 }).addTo(layer);
    ...
    if (up) up.textContent = data.available ? 'Обновлено ' + new Date(data.updated).toISOString().slice(11, 16) + 'z' : 'Сервер не отвечает';
