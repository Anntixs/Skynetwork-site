...
public sealed partial class BookingsModel(ContentService content, CurrentUser me) : PageModel
{
    ...
    public IActionResult OnPost(string callsign, string date, string from, string to)
    {
        ...
        if (!Position().IsMatch(callsign)) Error = this.T("Position: for example UUEE_TWR or UUWV_CTR");
        else if (start == null || end == null) Error = this.T("Enter the date and time");
        else if (content.Book(me.Cid, callsign, start.Value, end.Value) is { } error) Error = this.T(error, callsign);
        if (Error != null)
        {
            OnGet(null);
            return Page();
